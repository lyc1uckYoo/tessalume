internal static partial class TestSuite
{
    ...
    static async Task ReleaseReadinessAssetsAreDocumentedAsync()
    {
        ...
        var thirdPartyNoticesPath = Path.Combine(repositoryRoot, "THIRD-PARTY-NOTICES.md");
        ...
        var thirdPartyNotices = await File.ReadAllTextAsync(thirdPartyNoticesPath);
        ...
        Ensure(File.Exists(securityPath) && File.Exists(issueTemplatePath) && File.Exists(changelogPath) &&
               ...
               thirdPartyNotices.Contains("Apache License", StringComparison.Ordinal),
    ...
    static async Task GitHubAutomationSeparatesApplicationAndCompatibilityReleasesAsync()
    {
        ...
        var deltaPackScript = await File.ReadAllTextAsync(Path.Combine(
            repositoryRoot,
            "tools",
            "New-DeltaUpdatePack.ps1"));
        ...
        Ensure(release.Contains("tags:", StringComparison.Ordinal) &&
               ...
               release.Contains("New-DeltaUpdatePack.ps1", StringComparison.Ordinal) &&
               release.Contains("Tessalume.update.json", StringComparison.Ordinal) &&
               release.Contains("*.delta", StringComparison.Ordinal) &&
        ...
        Ensure(deltaPackScript.Contains("Tessalume.UpdatePack", StringComparison.Ordinal) &&
            ...
            "Incremental release assets must be generated by the bounded repository-owned pack builder.");
        ...
        foreach (var (relativePath, maximumLines) in new[]
                 {
                     ...
                     (Path.Combine("src", "Tessalume.Core", "Updates", "ReleaseUpdateClient.cs"), 350),
                     (Path.Combine("src", "Tessalume.Core", "Updates", "ReleaseUpdateClient.Download.cs"), 350),
                     (Path.Combine("src", "Tessalume.Core", "Updates", "ReleaseUpdateClient.Manifest.cs"), 350),
