...
从 1.3 起，推荐从软件内的“创作项目中心”管理主题。工作区的 `themes/` 下每个一级目录视为一个项目；Tessalume 会读取项目元数据，并按清单、入口、11 张模板素材、亮暗预览、Template 1.0、CSS、脚本和资源引用生成结构化体检。
...
亮暗原图以及设置基础 `background-position` / `background-size`，不得在这些图片
层上写死 `filter`、`opacity` 或 CSS 独立 `translate` / `scale`。Tessalume 会按主题、
亮暗模式分别持久化亮度、对比度、饱和度、不透明度、缩放、水平/垂直位置、灰度、
色相与柔化；默认值直接保留主题原图和基础裁切。运行时使用独立变换属性追加用户
构图，不会覆盖主题为角色基线设置的 `transform`。文字可读性遮罩必须放在独立层，
不能与原图共用滤镜。

用户可以把当前亮色或暗色下三个区域的参数保存为个人方案，并导出为
`.tessalume-look.json` 在不同主题或不同设备间复用。方案文件只保存经过范围校验的
数值参数和方案名称，不包含主题图片、脚本、主题 ID 或本机路径，因此它不是主题包，
也不会改变主题的原始 CSS 与资源。
