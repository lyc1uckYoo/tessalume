    ...
    ThemePayloadBuilder payloadBuilder) : IAsyncDisposable
{
    ...
    public async Task RemoveAsync(int port, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var target in targets)
        {
            ...
            await session.EvaluateAsync(
                "(async () => { await window.__CODEX_THEME_STUDIO_RUNTIME__?.dispose?.(); window.__CODEX_DREAM_SKIN_STATE__?.cleanup?.(); delete window.__CODEX_THEME_STUDIO_THEME_ID__; return true; })()",
                cancellationToken);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            foreach (var target in targets)
            {
                ...
                {
                    await session.EvaluateAsync(
                        $"window.__CODEX_THEME_STUDIO_THEME_ID__ = {JsonSerializer.Serialize(themeId)}; true",
                        cancellationToken);
                ...
                if (!force)
                {
                    var marker = await session.EvaluateAsync("window.__CODEX_THEME_STUDIO_THEME_ID__ || null", cancellationToken);
    ...
    {
        await session.EvaluateAsync(
            "window.__CODEX_THEME_STUDIO_STAGED_ASSETS__ = Object.create(null); true",
            cancellationToken);
        try
        {
            foreach (var (name, path) in assetPaths)
            {
                ...
                var expression =
                    $"window.__CODEX_THEME_STUDIO_STAGED_ASSETS__['{name}'] = '{dataUrl}'; true";
        ...
        catch
        {
            ...
            {
                await session.EvaluateAsync(
                    "delete window.__CODEX_THEME_STUDIO_STAGED_ASSETS__; true",
