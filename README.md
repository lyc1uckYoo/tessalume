...
Tessalume 会先查找已经运行的 Codex 页面，再决定是否自行启动 Codex。自动发现包含 Codex++ 默认使用的 `127.0.0.1:9229`，因此由 Codex++ 启动的窗口可以继续保留插件和用户脚本，同时由 Tessalume 应用主题。若其他宿主使用不同端口，可在“运行与诊断 → Codex 连接偏好”填写优先端口；留空即可恢复自动发现。
...
- 主题运行依赖 Codex Desktop 的本机调试端口；异常时请先打开“运行与诊断”查看当前端口或设置其他本机宿主的优先端口。
