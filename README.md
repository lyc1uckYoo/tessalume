...
[下载 Tessalume 2.0.0](https://github.com/lyc1uckYoo/tessalume/releases/latest) · [问题反馈](https://github.com/lyc1uckYoo/tessalume/issues/new?template=bug-report.yml) · [版本记录](CHANGELOG.md) · [主题制作指南](THEMING.md) · [安全与隐私](SECURITY.md) · [MIT 许可证](LICENSE)
...
2.0 把页面兼容与软件更新从“随完整 EXE 一起维护”升级为两条可独立恢复的链路。Codex 主页面、左栏、输入框、消息区和设置页的结构识别统一由版本化兼容配置管理；官方兼容小包可以独立发现、校验、安装和回退，页面类名变化时不必重复下载整套程序。

完整软件更新现在会在替换 EXE 后等待新版本提交启动健康确认；未能正常启动时自动换回旧版，成功后保留一个经过 SHA-256 校验的上一版本恢复点，也可以在“关于与数据”中手动恢复。更新前会为版本化设置与状态建立校验快照：降级时先恢复旧版可读取的配置，较新配置另存本地恢复副本。GitHub CI、正式版本发布和非 latest 的兼容补丁发布已经分离，减少发布过程中的人工遗漏。

创作中心 2.0 现在按“工作区 → 创作流程 → 项目体检 → 运行验收 → 发布清单”分成五个独立页面。主题问题可以整理成受限的 Codex 修复提示词；运行验收会检查亮色、暗色、输入框、消息框，并真实切换 800 / 1200 / 1800 三档视口验证响应式布局，再恢复原窗口与明暗状态；任何警告或未完成项都会阻止导出分享 ZIP。Tessalume 仍不读取对话，也不会替用户自动发送或执行 Codex 任务。

个性化 2.0 支持为首页横幅、左栏图片和聊天背景分别设置亮暗模式本地图片、构图、滤镜、叠色、渐变、暗角与可读性保护；动效强度、字号和界面密度可以保存为完整体验方案。应用内部同步完成按功能纵切的模块化重构，兼容运行时拆成可独立维护、发布时自动组装的职责模块。

1.4.1 及更早版本可以直接通过软件内自动更新升级到 2.0.0。更新会把 EXE 与版本化配置作为同一个事务处理，不会删除或重置 `data/`、`themes/`、`Compatibility/`、`Templates/`、收藏、个性化参数或创作工作区。

## Tessalume 1.4.1

1.4.1 是面向 Codex 最新页面结构的紧急兼容修复。Codex 更新后重命名了原生输入框外框和底部按钮栏，导致旧版 Tessalume 无法继续命中主题输入区样式，表现为输入框退回深色原生背景、主题描边与内部控件样式消失。本版本在共享兼容运行时中同时识别新旧结构，所有现有主题无需修改即可恢复输入框背景、边框、光效、文字、按钮和输入框挂件定位。

1.4.0 及更早版本可以直接通过软件内自动更新升级到 1.4.1。更新仍然只替换 EXE，不会删除或重置 `data/`、`themes/`、`Compatibility/`、`Templates/`、收藏、个性化参数或创作工作区。
...
- **五阶段创作与运行验收**：按角色研究、素材生成、契约校验、亮暗验收和发布导出显示进度；独立验收页批量检查亮色、暗色、输入框、消息框与响应式布局，并把问题归入主题项目或运行时兼容。
...
- **独立兼容补丁**：页面识别、语义标记、自适应布局和清理恢复由模块化兼容引擎统一维护；官方小包通过固定清单与 SHA-256 校验后原子安装，失败会回退上一规则或内置基线。
- **便携数据与开机启动**：主题、收藏、运行状态和界面设置都保存在 `Tessalume.exe` 旁边；开机启动默认关闭，只有用户主动开启后才写入当前 Windows 用户的启动项，不需要管理员权限。
- **完整自动更新**：开启后每次启动检查一次官方 GitHub Releases，也可在“关于与数据”手动检查；新版本会展示说明，经确认后下载、校验 SHA-256、备份旧 EXE、安全替换并自动重新启动；新版启动异常时自动恢复，成功后保留一个可手动恢复的上一版本。
...
开启自动检查后，软件每次启动都会并行检查正式软件版本和官方页面兼容规则；发现完整版本时，左上角品牌区会出现红色“更新”提示，点击后才展示版本说明并进入下载与安装确认。兼容规则可以通过独立小包静默更新，不必为 Codex 页面类名变化重复下载完整 EXE。可在“关于与数据 → 应用行为”管理自动检查和开机启动，手动检查始终可用。

完整更新只替换 `Tessalume.exe`，不会删除 `data/`、用户主题或个性化参数。新 EXE 启动并完成本地主题目录初始化后会提交健康确认；若未能启动，更新助手会自动换回旧版。成功后，更新前 EXE 会作为经过 SHA-256 复核的本机恢复点保留，可在“关于与数据 → 应用行为”明确确认后手动恢复上一版本。

### 从 1.2.x / 1.3.0 / 1.4.x 升级到 2.0.0

打开“关于与数据 → 应用行为”，点击“立即检查”；确认版本说明后，Tessalume 会下载并校验 `2.0.0`、备份旧 EXE 与旧版可读配置、安全替换并重新启动。整个过程不会删除程序旁边的 `data/`、`themes/`、`Compatibility/`、`Templates/` 或创作工作区。首次读取旧设置时会显式迁移为 Schema 4；自动回退会同步恢复旧版配置，手动降级还会在 `data/backups/version-rollback/` 保留新版配置副本。
...
发布页同时提供 `SHA256SUMS.txt`，可用于核对下载的 EXE。Tessalume 2.0.0 当前面向安装了 Windows 版 Codex Desktop 的 x64 系统。当前构建没有商业代码签名，首次下载时 Microsoft Defender SmartScreen 可能显示提示；请只从本仓库 Releases 下载并核对 SHA-256。
...
- 下载的新 EXE 必须通过 Release 资源摘要或 `SHA256SUMS.txt` 校验；安装时先备份旧 EXE，新版启动健康检查失败会自动恢复旧版，成功后仍保留一个可手动恢复的上一版本；
- 页面兼容补丁只接受官方 `compat-vX.Y.Z` Release，校验压缩包与内部每个文件的 SHA-256、固定文件清单、体积、最低软件版本和运行时契约；补丁不会覆盖内置兼容基线；
...
Codex 会先展示角色身份卡和 11 张素材计划，确认后完成制作与校验。回到 Tessalume 后可以直接查看分组体检、打开问题文件、重新应用主题、切换 Codex 明暗预览，并把通过体检的项目导出为可分享 ZIP。项目存在错误或建议时，还可以把当前主题的结构化体检结果整理成一条修复提示词，再由用户自行交给 Codex；Tessalume 不会自动启动任务或发送内容。

## 已知边界与排查顺序

- 2.0.0 当前只发布 Windows x64 自包含单文件，没有 macOS、Linux 或 ARM64 构建。
...
仓库内置三条 GitHub Actions 流程：普通推送和 Pull Request 运行与本机相同的一键 Release 构建；推送与项目版本一致的 `vX.Y.Z` 标签会读取对应 CHANGELOG 段落并发布正式 EXE；推送 `compat-vX.Y.Z` 标签只生成页面兼容小包，并强制标记为非 latest，避免干扰软件正式版本检查。
...
Tessalume 2.0.0 的正式发布产物为 `Tessalume.exe` 与对应的 SHA-256 校验文件，不额外要求安装器或 .NET 运行环境。
